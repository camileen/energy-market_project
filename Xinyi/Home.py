...
class Home:
    ...
    if self.energy > self.energy_threshold:
      if self.energy_trade == GIVE_ONLY:
        ...
        try:
          ...
          self.send_market_sell_request(HOST, PORT)
    else: # lack of energy
      ...
      try:
        ...
        self.send_market_request(HOST, PORT)
        self.wait_response()
      except EnoughEnergy:
        pass
  ...
  def give(self):
    ...
    def send_market_sell_request(self, HOST, PORT):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
            client_socket.connect((HOST, PORT))
            message = [self.energy- self.energy_threshold, 1]
            data_send = struct.pack('2d', *message)
            client_socket.send(data_send)
            data = client_socket.recv(8)
            print("Client Revieve: ", struct.unpack('2d',data))

    def send_market_request(self, HOST, PORT):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
            client_socket.connect((HOST, PORT))
            message = [self.energy- self.energy_threshold, 0]
            data_send = struct.pack('2d', *message)
            client_socket.send(data_send)
            data = client_socket.recv(8)
            print("Client Revieve: ", struct.unpack('2d',data))
